...
struct recursion_state {
  ...
  bool operator==(const recursion_state &o) const {
    bool result = index == o.index;
    for (int i = 0; i < state.size(); i++) {
      result = result and state[i] == o.state[i];
    }
    return result;
  }

  bool operator<(const recursion_state &o) const {
    return index < o.index or (index == o.index and VectorToInt(state) < VectorToInt(o.state));
  }
};

map<recursion_state, int> memo;

int is_possible(int index, vector<int> &state,
      vector<vector<vector<int> > > &switch_effects) {

  recursion_state rec = {index, state};
  if (memo.find(rec) != memo.end()) {
    return memo[rec];
    cout << "used memoization." << endl;
  }
  ...
  int result1, result2;
  result1 = is_possible(index - 1, taken, switch_effects);
  result1 = (result1 >= 0) ? result1 : INT_MAX - 1;
  result2 = is_possible(index - 1, negated, switch_effects);
  result2 = (result2 >= 0) ? result2 : INT_MAX - 1;
  int result;
  if (min(result1, result2) == INT_MAX - 1) {
    result = -1;
  } else {
    result = min(result1, result2 + 1);
  }
  memo[rec] = result;
  return result;
}
